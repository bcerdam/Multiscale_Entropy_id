...
double calculate_U_ij_m(double ***list_of_matrices, int i, int j, int k, int m, double r, int H, int W, int T, double delta, int fuzzy, int distance_type, int window_size, int mod, int m_espacial, int dim_cubo, double r_0, double r_1, int std_type) {
    double count = 0;
    int t_reduce;
                        ...
                        if (window_size == 0){
                            ...
                            if (fuzzy == 0){
                                ...
                                if (std_type == 0){
                                    ...
                                    // if (dist <= r_0){
                                    count += fuzzy_membership(dist, r_0, delta);
                                        // count += fuzzy_sinusoidal(dist, r, delta);
                                    // }
                                }
                            }
                            ...
                            if (fuzzy == 0){
                                ...
                                if (std_type == 0){
                                    ...
                                    // if (dist <= r_1){
                                    count += fuzzy_membership(dist, r_1, delta);
                                        // count += fuzzy_sinusoidal(dist, r, delta);
                                    // }
                                }
                            }
...
double calculate_U_m(double ***list_of_matrices, int m, double r, int H, int W, int T, double delta, int fuzzy, int distance_type, int window_size, int mod, int m_espacial, int dim_cubo, int n_threads, double r_0, double r_1, int std_type) {
    double sum = 0.0;
    int t_reduce;
    ...
    for (int k = 0; k < T - t_reduce; k++){
        for (int i = 0; i < H - m; i++) {
            for (int j = 0; j < W - m; j++) {
                sum += calculate_U_ij_m(list_of_matrices, i, j, k, m, r, H, W, T, delta, fuzzy, distance_type, window_size, mod, m_espacial, dim_cubo, r_0, r_1, std_type);
            }
        }
    }
    #pragma omp barrier
...
double* method_mse(double*** list_of_matrices, int scales, int m, double r, int fuzzy, int method, double delta, int distance_type, int num_files, int rows, int cols, int mod, int m_espacial, int dim_cubo, int n_threads, double r_0, double r_1, int std_type){
    ...
    // for (int i = 18; i <= 20; i++) {  
        if (method == 1 || method == 2) {
            ...
            for (int j = 0; j <= coarse_grained_n; j++) { 
                ...
                float U_m = calculate_U_m(coarse_data, m, r, cols, rows, (num_files - coarse_grained_n) / i, delta, fuzzy, distance_type, 0, mod, m_espacial, dim_cubo, n_threads, r_0, r_1, std_type);
                float U_m_plus_one = calculate_U_m(coarse_data, m, r, cols, rows, (num_files - coarse_grained_n) / i, delta, fuzzy, distance_type, 1, mod, m_espacial, dim_cubo, n_threads, r_0, r_1, std_type);
            ...
            double U_m = calculate_U_m(coarse_data, m ,r, rows, cols, num_files/i, delta, fuzzy, distance_type, 0, mod, m_espacial, dim_cubo, n_threads, r_0, r_1, std_type);
            double U_m_plus_one = calculate_U_m(coarse_data, m, r, rows, cols, num_files/i, delta, fuzzy, distance_type, 1, mod, m_espacial, dim_cubo, n_threads, r_0, r_1, std_type);
...
int main(int argc, char* argv[]) {
    ...
    if(std_type == 1){
        ...
        double signal_std_ws_0 = distance_m(signal_array, m_distance, num_rows, num_cols, num_matrices, distance_type, sampleo, m_temp, 0);
        double signal_std_ws_1 = distance_m(signal_array, m_distance, num_rows, num_cols, num_matrices, distance_type, sampleo, m_temp, 1);
        r_0 *= signal_std_ws_0;
        r_1 *= signal_std_ws_1;
    }
    else{
    ...
    // MSE 3D
    double* entropy_values = method_mse(signal_array, scales, m, r, fuzzy, method, delta, distance_type, num_matrices, num_rows, num_cols, mod, m_espacial, dim_cubo, n_threads, r_0, r_1, std_type);
