...
def run_c_program(csv_path, scales, m, r, fuzzy, method, delta=0.7, distance_type=0, m_distance=2, std_type=1, dim=1, n_threads=32):
    ...
    command = [os.path.join(os.path.dirname(os.getcwd()), 'Multiscale_Entropy_id/core_c/mse_1d', 'executables', 'mse_1d_p'), csv_path, str(scales), str(m), str(r), str(fuzzy), str(method),
