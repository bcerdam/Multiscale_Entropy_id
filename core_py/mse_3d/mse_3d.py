...
parameters:
    - Same from mse_3d.
'''
def run_c_program(input, scales, m, r, fuzzy, method, delta=0.7, distance_type=0, m_distance=2, sampleo=1, std_type=1, mod=False, m_espacial=1, dim_cubo=1, n_threads=32, m_temp=2, plus_one=False):
    info = backbone.info_matriz(input)

    # Non cluster
    command = [os.path.join(os.path.dirname(os.getcwd()), 'Multiscale_Entropy_id/core_c/mse_3d', 'executables', 'mse_3d_p'), input, str(scales), str(m), str(r), str(fuzzy), str(method),
               str(delta), str(distance_type), str(m_distance), str(sampleo), str(info[0]), str(info[1]), str(info[2]), str(std_type), str(mod), str(m_espacial), str(dim_cubo), str(n_threads), str(m_temp), str(plus_one)]
...
parameters:
    ...
    - m_distance (int): If 'UNIQUE_DISTANCES' is used, then this parameter is equivalent to the 'm' parameter. Height and width
    - m_temp (int): If 'UNIQUE_DISTANCES' is used, then this parameter defines how many steps forward we consider.
    - plus_one (bool): if 'UNIQUE_DISTANCES' is used, if False then distance wont consider plus one data point, True the opposite.
...
'''
def mse_3d(input, scales, m, r, fuzzy, method, delta=0.7, distance_type=0, m_distance=2, sampleo=1, std_type='UNIQUE_VALUES', mod=False, m_espacial=1, dim_cubo=1, shape=(10, 10, 100), n_threads=32, status=False, m_temp=2, plus_one=False):
    if status:
    ...
    if input_type == 'np.array':
        ...
        mse_values.append(['array',
                           run_c_program(filepath[1], scales, m, r, fuzzy, method, delta, distance_type, m_distance,
                                         sampleo, std_type, mod, m_espacial, dim_cubo, n_threads, m_temp, plus_one)])
        ...
        for filename in os.listdir(input):
            ...
            mse_values.append([filename[:-4], run_c_program(file_path, scales, m, r, fuzzy, method, delta, distance_type, m_distance, sampleo, std_type, mod, m_espacial, dim_cubo, n_threads, m_temp, plus_one)])
        ...
        for filename in os.listdir(input):
            if '.mp4' not in filename:
                ...
                mse_values.append([filename, run_c_program(csv_filepath[1], scales, m, r, fuzzy, method, delta, distance_type, m_distance, sampleo, std_type, mod, m_espacial, dim_cubo, n_threads, m_temp, plus_one)])
